...
<body class="blog-page">
  ...
  <main class="main">
    ...
    if (isset($_GET['id'])) {
      $id = intval($_GET['id']);
      $stmt = $conn->prepare("SELECT title, blog_message, created_at, image FROM blog_tb WHERE id = ?");
      $stmt->bind_param("i", $id);
      $stmt->execute();
      $stmt->store_result();

      if ($stmt->num_rows > 0) {
        $stmt->bind_result($title, $message, $created_at, $image);
        $stmt->fetch();
      } else {
      ...
      echo "<p class='text-danger'>No blog ID specified.</p>";
      exit;
    }
          ...
          <section id="blog-details" class="blog-details section">
            <div class="container">
              <article class="article">
                ...
                <div class="content">
                  <p class="text-dark"><?= nl2br(htmlspecialchars($message)) ?></p>
